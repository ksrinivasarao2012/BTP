...
def evaluate_single_config_p2(args):
    ...
    drone_succ_rate = (
        successes / evaluated_drones
        if evaluated_drones > 0 else 0
    )

    ep_succ_rate = (
        episode_successes / total_episodes
        if total_episodes > 0 else 0
    )

    coll_rate = (
        collisions / evaluated_drones
        if evaluated_drones > 0 else 0
    )

    timeout_rate = (
        timeouts / evaluated_drones
        if evaluated_drones > 0 else 0
    )
    ...
    return {
        ...
        "Mean_Episode_Length":
            np.mean(ep_lengths) if ep_lengths else 0,

        "Mean_Time_To_Goal":
            np.mean(times_to_goal) if times_to_goal else 0,

        "Mean_Escape_Triggers":
            np.mean(escape_triggers) if escape_triggers else 0
    }
...
def run_phase2():
    ...
    tasks = [
        (w, h, density, d_min, num_episodes)
        for (w, h, density, d_min)
    ...
    print(
        f"Starting Phase 2 Validation "
        f"({len(tasks)} configurations, "
    ...
    # Same Phase 3 selection criterion
    phase2_survivors = df[
        (df["Drone_Success_Rate"] >= 0.40)
        &
        (df["Drone_Success_Rate"] <= 0.80)
    ]

    phase2_survivors.to_csv(
        "results/phase2/"
        "phase2_validation_survivors.csv",
        index=False
    )
    ...
    for w in df["Width"].unique():
        ...
        pivot_success = sub.pivot(
            index="Density",
            columns="d_min",
            values="Drone_Success_Rate"
        )
        ...
        ax.set_xticks(
            np.arange(len(pivot_success.columns))
        )

        ax.set_yticks(
            np.arange(len(pivot_success.index))
        )
        ...
        for i in range(len(pivot_success.index)):
            for j in range(len(pivot_success.columns)):
                ...
                color = (
                    "black"
                    if (val if pd.notnull(val) else 0) > 0.5
                    else "white"
                )
        ...
        ax.set_title(
            f"Reactive Baseline Drone Success Rate "
            f"(Size {w})"
        )
        ...
        plt.savefig(
            f"plots/difficulty/"
            f"drone_success_rate_size_{w}.png"
        )
