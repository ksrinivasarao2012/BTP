"""
Eval-time congestion ablation on the EXISTING comm=8 model (no retraining).

Loads v14_8.0m and evaluates it with communication_range = 8.0 (unchanged) but
use_congestion = False (congestion dim forced to 0). This measures how much the
ALREADY-TRAINED model relies on the congestion feature.

NOTE: this is an eval-time ablation -> it tests DEPENDENCE of this trained model,
not whether the task NEEDS congestion (that would require retraining 'nocong').
...
COMM = 8.0
USE_CONGESTION = False   # <-- the ablation
...
def main():
    ...
    print(f"[*] CONGESTION ABLATION (eval-time) on existing model: {MODEL_PATH}")
    print(f"[*] comm={COMM}m, use_congestion={USE_CONGESTION} (no retraining)")
    ...
    for density in DENSITIES:
        env = SwarmLidarEnv_StepB10_8_0m(render_mode=None, target_density=density,
                                         communication_range=COMM, use_congestion=USE_CONGESTION)
        ...
        results.append(row)
        print(f"[*] 8m no-congestion (eval-time) d={density:.2f}: success {row['success_rate']*100:.2f}% | "
    ...
    out = os.path.join(out_dir, "comm8_nocong_evaltime_metrics.csv")
    pd.DataFrame(results).to_csv(out, index=False)
    print(f"\n[OK] saved: {out}")
    print("Compare against comm=8 WITH congestion: 95.45% / 91.4%.")
