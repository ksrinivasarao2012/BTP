...
# ======================================================
#  PHASE B10 v14: Adaptive Wall-Glide Fine-Tuning
#  Fine-tunes from B9 v13 weights in the new B10 environment.
# ======================================================
...
class MultiProcessPZEnv_B10(VecEnv):
...
def run_v14_training():
    num_cpu = 10
    v13_model_path = "models/apex_ultra_topological_v13_final.zip"
    
    # Robust path resolution
    possible_paths = [
        v13_model_path,
        "../../models/apex_ultra_topological_v13_final.zip",
        "../../../models/apex_ultra_topological_v13_final.zip",
        os.path.join(os.path.dirname(__file__), "../../models/apex_ultra_topological_v13_final.zip"),
        os.path.join(os.path.dirname(__file__), "../../../models/apex_ultra_topological_v13_final.zip")
    ]
    ...
    print(f"PHASE B10 v14: Adaptive Wall-Glide Fine-Tuning")
    print(f"  -> Loading pre-trained v13 weights from: {v13_model_path}")

    # Create the new B10 environment
    base_env = MultiProcessPZEnv_B10(n_workers=num_cpu, density=0.35)
    env = VecNormalize(base_env, norm_obs=False, norm_reward=True, clip_reward=10.0)

    # Load pre-trained v13 model and attach to the new environment
    model = PPO.load(
        v13_model_path,
        env=env,
    ...
    # Lower the learning rate for precise fine-tuning
    model.learning_rate = 8e-5
    model.ent_coef = 0.015  # Keep exploration active but refined

    print(f"  -> v13 weights loaded successfully. Fine-tuning in B10 environment...")
    ...
    os.makedirs("./models/checkpoints_b10v14/", exist_ok=True)
    checkpoint_callback = CheckpointCallback(
        save_freq=500_000,
        save_path='./models/checkpoints_b10v14/',
        name_prefix='b10v14'
    )
    ...
    for steps, density in curriculum:
        ...
        model.save(f"./models/apex_ultra_glide_v14_mid_{steps//1_000_000}M")

    model.save("./models/apex_ultra_glide_v14_final")
    env.save("./models/vecnormalize_glide_v14_final.pkl")
    env.close()
    print(f"\nPhase B10 v14 Training Complete (5M fine-tune steps). Model saved.")
