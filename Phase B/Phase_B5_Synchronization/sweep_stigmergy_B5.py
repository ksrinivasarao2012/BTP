...
def evaluate_config_worker(args):
    ...
    model = PPO.load(model_path, device="cpu", custom_objects={"policy_class": MAPPO_Policy_B5})
...
def run_grid_sweep(model_path, cores=8):
    print("\n" + "="*70)
    print("STARTING STIGMERGY B5 HYPERPARAMETER SWEEP")
    print("="*70)
...
if __name__ == "__main__":
    # We use v14 Glide or B5 sync final as the starting point model
    # Wait, our unoptimized baseline model is `stigmergy_b5_model_35.zip`.
    # But wait, in Phase_B5_Synchronization we don't have stigmergy_b5_model_35 yet!
    # Let's perform the grid sweep using the unoptimized stigmergy model we had in the other folder!
    # Wait, since the other folder has `stigmergy_b5_model_35.zip`, we can copy it or reference it!
    # Or, we can do weight surgery on the fly to run the sweep on the reactive model!
    # Wait, doing weight surgery on the fly during the sweep is very clean, or we can just pass the path to the model!
    
    default_model = "../Phase_B_Stigmergy/stigmergy_b5_model_35.zip"
    m_path = sys.argv[1] if len(sys.argv) > 1 else default_model
    c_count = int(sys.argv[2]) if len(sys.argv) > 2 else 8
    
    if not os.path.exists(m_path):
        # Fallback to checking local models
        if os.path.exists("./models/apex_ultra_glide_v14_final.zip"):
            m_path = "./models/apex_ultra_glide_v14_final.zip"
        else:
            m_path = "../../models/v15_Master_Recovered_Final.zip"
