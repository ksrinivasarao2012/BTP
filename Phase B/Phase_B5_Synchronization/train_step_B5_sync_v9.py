...
# ======================================================
#  PHASE B5 v9: IEEE Q1 Certification (Hardening)
# ======================================================
...
def run_v9_training():
    num_cpu = 10 
    print(f"PHASE B5 v9: IEEE Q1 Baseline (Sensing Radii + Decompression)")
    ...
    model = PPO.load(model_path, env=env, ent_coef=0.03, learning_rate=5e-5, custom_objects=custom_objects)
    
    # 10M Step Hardening Curriculum
    curriculum = [
        (3_000_000, 0.25),   # Phase 1: Re-synchronize under limited sensing
        (3_000_000, 0.30),   # Phase 2: Learn Swarm Decompression (Reverse Shockwave)
        (4_000_000, 0.35),   # Phase 3: High-Density Hardening
    ]
    total_steps = 0
    os.makedirs("./models/checkpoints_b5v9/", exist_ok=True)
    checkpoint_callback = CheckpointCallback(save_freq=500_000, save_path='./models/checkpoints_b5v9/', name_prefix='b5v9_q1')
    ...
    for steps, density in curriculum:
        ...
        total_steps += steps
        model.save(f"./models/apex_ultra_sync_v9_mid_{total_steps//1000}k")
    
    model.save("./models/apex_ultra_sync_v9_final")
    env.save("./models/vecnormalize_sync_v9_final.pkl")
    env.close()
    print(f"\nPhase B5 v9 Training Complete ({total_steps/1e6:.1f}M steps). Model saved.")
