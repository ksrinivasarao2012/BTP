...
from swarm_env_step_B7 import SwarmLidarEnv_StepB7
from multiprocessing import Process, Pipe

# ======================================================
#  PHASE B7 v11: Patience Decay + Deadlock Recovery
#  Fine-tunes from v10 weights in the new B7 environment
#  so the agent learns to break out of blocked queues.
# ======================================================
...
# -----------------------------------------------------------
# Subprocess Worker – uses SwarmLidarEnv_StepB7
# -----------------------------------------------------------
def worker(remote, parent_remote, density, drone_radius, safety_radius):
    ...
    os.environ["OMP_NUM_THREADS"] = "1"
    parent_remote.close()
    env = SwarmLidarEnv_StepB7(render_mode=None, target_density=density)
    n_drones = 10
    ghost_obs = {}
    try:
        while True:
...
class MultiProcessPZEnv_B7(VecEnv):
...
def run_v11_training():
    num_cpu = 10
    v10_model_path = "./models/apex_ultra_queue_v10_final.zip"

    print(f"PHASE B7 v11: Patience Decay Fine-Tuning")
    print(f"  -> Loading pre-trained v10 weights from: {v10_model_path}")

    # Create the new B7 environment
    base_env = MultiProcessPZEnv_B7(n_workers=num_cpu, density=0.35)
    env = VecNormalize(base_env, norm_obs=False, norm_reward=True, clip_reward=10.0)

    # Load pre-trained v10 model and attach to the new environment
    # This gives the agent a massive head-start — it already knows navigation,
    # obstacle avoidance, and smart yielding. It only needs to learn
    # how to break deadlocks (the patience decay behaviour).
    model = PPO.load(
        v10_model_path,
        env=env,
    ...
    # Lower the learning rate for fine-tuning (preserve existing knowledge)
    model.learning_rate = 1e-4
    model.ent_coef = 0.02  # Slightly lower entropy for fine-tuning stability

    print(f"  -> v10 weights loaded successfully. Fine-tuning in B7 environment...")

    # Shorter curriculum: only 5M steps needed since we're fine-tuning
    # Phase 1: 2M @ 0.30 density — warm up in easier scenarios
    # Phase 2: 3M @ 0.35 density — full difficulty with patience decay active
    curriculum = [
        (2_000_000, 0.30),   # Phase 1: Gentle warm-up for reward signal adaptation
        (5_000_000, 0.35),   # Phase 2: Full-density deadlock recovery training
    ]

    os.makedirs("./models/checkpoints_b7v11/", exist_ok=True)
    checkpoint_callback = CheckpointCallback(
        save_freq=500_000,
        save_path='./models/checkpoints_b7v11/',
        name_prefix='b7v11'
    )
    ...
    for steps, density in curriculum:
        ...
        model.save(f"./models/apex_ultra_patience_v11_mid_{steps//1_000_000}M")

    model.save("./models/apex_ultra_patience_v11_final")
    env.save("./models/vecnormalize_patience_v11_final.pkl")
    env.close()
    print(f"\nPhase B7 v11 Training Complete (5M fine-tune steps). Model saved.")
