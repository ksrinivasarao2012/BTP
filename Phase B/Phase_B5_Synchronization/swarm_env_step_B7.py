...
# ======================================================
#  PHASE B7 v11: SMART QUEUING + PATIENCE DECAY
#  Adds deadlock recovery: after 100 blocked steps the
#  patience reward decays linearly and stagnation counter
#  is unfrozen, forcing the agent to steer around.
# ======================================================
class SwarmLidarEnv_StepB7(ParallelEnv):
    metadata = {'render_modes': ['human'], "name": "swarm_lidar_stepB7_v0"}
    ...
    def __init__(self, render_mode=None, target_density=0.20, drone_radius=0.15, safety_radius=0.19):
        ...
        # [B7] Patience-decay deadlock tracker (per-agent)
        self.blocked_steps = {}
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            if dist_goal < self.best_dist_to_goal[agent] - 0.1:
                ...
                self.steps_stagnant[agent] = 0
                self.blocked_steps[agent] = 0  # [B7] progress resets blocked counter too
            elif is_blocked:
                # [B7 PATIENCE DECAY] ---- replaces the B6 unconditional freeze ----
