...
# ======================================================
#  PHASE B8 v12: POSITIONAL HISTORY STAGNATION
#  Replaces velocity stagnation checks with a 50-step 
#  positional history window to detect micro-oscillations.
# ======================================================
class SwarmLidarEnv_StepB8(ParallelEnv):
    metadata = {'render_modes': ['human'], "name": "swarm_lidar_stepB8_v0"}
    ...
    def __init__(self, render_mode=None, target_density=0.20, drone_radius=0.15, safety_radius=0.19):
        ...
        # [B8] Positional history stagnation deadlock tracker
        self.blocked_steps = {}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.blocked_steps = {agent: 0 for agent in self.possible_agents}  # [B7] reset blocked tracker
        self.position_history = {agent: deque(maxlen=5) for agent in self.possible_agents}
        self.stagnation_position_history = {agent: deque(maxlen=50) for agent in self.possible_agents}
        ...
        # Seed initial best distances and stagnation history
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            # 2. Advanced Stagnation Logic with Smart Queuing Yield (B8 Positional)
            speed = np.linalg.norm(self.velocities[idx])
            ...
            if dist_goal < self.best_dist_to_goal[agent] - 0.1 and not is_stagnant:
                ...
                if is_blocked_by_drone:
                    ...
                    if blk <= 100:
                        # Phase 1 (first 10 s): original B6 smart-yield behaviour
                        self.steps_stagnant[agent] = 0
                        if speed < 0.2:
                            rewards[agent] += 0.5  # patience reward
                    else:
                        # Phase 2 (10-20 s): decay patience & unfreeze stagnation
                        decay = max(0.0, 1.0 - (blk - 100) / 100.0)
                        if speed < 0.2:
                            rewards[agent] += 0.5 * decay
                        # Stagnation counter now ticks => frustration penalty kicks in
                        self.steps_stagnant[agent] += 1
                    if blk > 150:
                        # Phase 3 (>15 s): extra nudge to break the deadlock
                        rewards[agent] -= 0.15 * min((blk - 150) / 50.0, 1.0)
                else:
                    self.steps_stagnant[agent] += 1
                    self.blocked_steps[agent] = 0
