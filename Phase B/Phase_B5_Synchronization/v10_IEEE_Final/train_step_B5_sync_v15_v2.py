...
# ============================================================
#  PHASE B MASTER v15 — VERSION 2 (Dijkstra + Warm-Init Synchronized)
#  Key improvements:
#  [1] LogStdClampCallback  — prevents variance explosion
#  [2] Exact [64, 64] arch  — ensures 100% warm-init weight transfer from v14!
#  [3] Paced Curriculum     — 5M steps to optimize dense navigation
#  [4] Tighter clip_range   — prevents catastrophic policy updates
#  [5] Dynamic Wall-Glide   — resolved inside the environment
#  [6] Denied-GPS reference — fully relative state estimation
# ============================================================

# ==============================================
#  TRAINING CONFIG
# ==============================================
TOTAL_TIMESTEPS   = 5_000_000    # Optimized for warm-init convergence
NUM_WORKERS       = 10           # CPU cores
N_STEPS           = 2048         # Steps per rollout per worker
BATCH_SIZE        = 2048         # Mini-batch size
N_EPOCHS          = 10
LEARNING_RATE     = 1e-4         # Low LR to refine spatial pathing gently
ENT_COEF          = 0.015        # Promotes robust exploration of communications dims
CLIP_RANGE        = 0.15         # Prevents large policy drift updates
GAMMA             = 0.995
GAE_LAMBDA        = 0.95
MAX_GRAD_NORM     = 0.5
LOG_STD_MIN       = -2.5         # exp(-2.5) = 0.08 (tight exploration control)
LOG_STD_MAX       = 0.0          # exp(0.0)  = 1.0
CHECKPOINT_FREQ   = 500_000

# Path configuration
WARM_INIT_MODEL   = r"d:\Swarm\BTP\models\apex_ultra_glide_v14_final.zip"
CHECKPOINT_DIR    = r"d:\Swarm\BTP\models\v15_v2_checkpoints"
FINAL_MODEL_PATH  = r"d:\Swarm\BTP\models\v15_Master_v2_Final.zip"
NORM_SAVE_PATH    = r"d:\Swarm\BTP\models\v15_Master_v2_Normalize.pkl"

# ==============================================
#  POLICY ARCHITECTURE (732D: 202D + 530D)
#  Hidden layer architecture set to [64, 64]
#  to perfectly mirror the pre-trained B10 baseline!
# ==============================================
...
# ==============================================
#  CALLBACK 1: Log_std Clamp (anti-drift guard)
# ==============================================
...
# ==============================================
#  CALLBACK 2: Curriculum (Paced for 5M steps)
# ==============================================
class CurriculumCallback_v2(BaseCallback):
    STAGE1_END = 1_000_000
    STAGE2_END = 2_500_000
    STAGE3_END = 4_000_000
    ...
    def _on_step(self) -> bool:
        ...
        if ts <= self.STAGE1_END:
            ...
            density  = 0.20 + frac * 0.05     # 0.20 -> 0.25
        elif ts <= self.STAGE3_END:
            frac = (ts - self.STAGE2_END) / (self.STAGE3_END - self.STAGE2_END)
            r_comm, r_sensor = 10.0, 8.0
            density = 0.25 + frac * 0.05      # 0.25 -> 0.30
        else:
            frac = min(1.0, (ts - self.STAGE3_END) / (TOTAL_TIMESTEPS - self.STAGE3_END))
            r_comm, r_sensor = 10.0, 8.0
            density = 0.30 + frac * 0.05      # 0.30 -> 0.35
        ...
        if (abs(r_comm - self._last_r_comm) > 0.5 or
            ...
            print(f"  [Curriculum] Step {ts:,}: R_comm={r_comm:.1f}m, "
                  f"R_sensor={r_sensor:.1f}m, Density={density:.2f}", flush=True)
        return True

# ==============================================
#  WARM INIT: Transfer v14 (650D) -> v15 (732D)
#  Copies all [64, 64] hidden layers exactly.
#  Expands first actor layer: 130D -> 202D (zero-pads new dims)
#  Expands first critic layer: 520D -> 530D (zero-pads new dims)
#  Result: 100% of spatial knowledge successfully transferred!
# ==============================================
def warm_init_from_v14(model, v14_path):
    print(f"\n  [WarmInit] Loading v14 weights from: {v14_path}", flush=True)
    import zipfile, io
    ...
    for k, v14_tensor in v14_params.items():
        ...
        if v14_tensor.shape == v15_tensor.shape:
            # Exact match — copy directly
            v15_sd[k] = v14_tensor.clone()
            transferred += 1

        elif len(v14_tensor.shape) == 2 and v14_tensor.shape[0] == v15_tensor.shape[0]:
            # First layers: copy columns, zero-pad/randomize newly expanded dimensions
            new_w = v15_sd[k].clone()
            n_old = v14_tensor.shape[1]
            new_w[:, :n_old] = v14_tensor
            # Slightly damp the new extra dimensions to avoid instability
            nn.init.normal_(new_w[:, n_old:], mean=0.0, std=0.01)
            v15_sd[k] = new_w
            transferred += 1
    ...
    print(f"  [WarmInit] Transferred: {transferred} tensors | Skipped: {skipped} tensors", flush=True)
    print(f"  [WarmInit] Initial log_std clamped to -1.5 (std=0.22)\n", flush=True)

# ==============================================
#  VECTORIZED MULTI-PROCESS ENVIRONMENT
# ==============================================
...
# ==============================================
def run_v15_v2_training():
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    os.makedirs(os.path.dirname(FINAL_MODEL_PATH), exist_ok=True)

    print("\n" + "="*70, flush=True)
    print("  V15 MASTER v2 — DIJKSTRA & WARM-INIT SYNCHRONIZED TRAINING", flush=True)
    print("="*70, flush=True)
    ...
    print(f"  LogStd Clamp   : [{LOG_STD_MIN}, {LOG_STD_MAX}]", flush=True)
    ...
    policy_kwargs = dict(
        net_arch=dict(pi=[64, 64], vf=[64, 64]), # Set to [64, 64] to ensure perfect weight transfer!
    ...
    print("[3/4] Applying warm initialization from v14...", flush=True)
    if os.path.exists(WARM_INIT_MODEL):
        warm_init_from_v14(model, WARM_INIT_MODEL)
    else:
        print(f"  [WarmInit] WARNING: v14 model not found at {WARM_INIT_MODEL}", flush=True)
        print(f"  [WarmInit] Starting from random initialization.", flush=True)
        if hasattr(model.policy, "log_std"):
            with torch.no_grad():
                model.policy.log_std.data.fill_(-1.5)
    ...
    curriculum_cb = CurriculumCallback_v2()

    print("[4/4] Starting training run...\n", flush=True)
    model.learn(
