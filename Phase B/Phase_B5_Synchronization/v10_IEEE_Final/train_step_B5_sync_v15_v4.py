...
# ============================================================
#  PHASE B MASTER v15 — VERSION 4 (Exact B10 Spawn & Safety Synchronized)
#  Key improvements:
#  [1] Warm-initialization directly from v15 Master v3 Final!
#  [2] Exact B10 Spawn logic (0.6m min spacing, random scatter fallback, push-out).
#  [3] Focused 500,000 step refinement training directly locked at 0.35 target.
#  [4] Low Learning Rate (3e-5) & low entropy (0.005) for gentle adjustment.
# ============================================================

# ==============================================
#  TRAINING CONFIG
# ==============================================
TOTAL_TIMESTEPS   = 500_000      # Short focused run to refine safety behavior on new spawn rules
NUM_WORKERS       = 10           # CPU cores
N_STEPS           = 2048         # Steps per rollout per worker
BATCH_SIZE        = 2048         # Mini-batch size
N_EPOCHS          = 10
LEARNING_RATE     = 3e-5         # Low learning rate to safely adjust weights
...
# Path configuration
WARM_INIT_MODEL   = r"d:\Swarm\BTP\models\v15_Master_v3_Final.zip"
CHECKPOINT_DIR    = r"d:\Swarm\BTP\models\v15_v4_checkpoints"
FINAL_MODEL_PATH  = r"d:\Swarm\BTP\models\v15_Master_v4_Final.zip"
NORM_SAVE_PATH    = r"d:\Swarm\BTP\models\v15_Master_v4_Normalize.pkl"
...
class CurriculumCallback_v4(BaseCallback):
...
# Precise identical 100% parameter loading from v15 Master v3
def warm_init_from_v15_v3(model, v15_v3_path):
    print(f"\n  [WarmInit] Loading v15_v3 weights from: {v15_v3_path}", flush=True)
    import zipfile, io
    with zipfile.ZipFile(v15_v3_path) as z:
...
def run_v15_v4_training():
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    os.makedirs(os.path.dirname(FINAL_MODEL_PATH), exist_ok=True)

    print("\n" + "="*70, flush=True)
    print("  V15 MASTER v4 — EXACT B10 SPAWN & SAFETY REFINE RUN", flush=True)
    print("="*70, flush=True)
    ...
    print("[3/4] Applying warm initialization from v15 Master v3...", flush=True)
    if os.path.exists(WARM_INIT_MODEL):
        warm_init_from_v15_v3(model, WARM_INIT_MODEL)
    else:
    ...
    curriculum_cb = CurriculumCallback_v4()
