...
# ======================================================
#  PHASE B v20 SENSING ABLATION: 64-RAY LIDAR + MLP
#  Architecture: 366D Actor | 1970D Critic
...
#  Changes from v19: 64-ray LiDAR, max_velocity=1.2
# ======================================================

class SwarmLidarEnv_v20_SensingAblation(ParallelEnv):
    metadata = {'render_modes': ['human'], "name": "swarm_lidar_v20_sensing_ablation"}

    def __init__(self, render_mode=None, target_density=0.15, width=20.0, height=20.0):
        super().__init__()
        self.n_drones = 10
        self.max_steps = 800
        ...
        # v20: 64-ray LiDAR = 192D (64 sectors x 3 values)
        # Actor: 7D self + 192D lidar + 144D neighbors + 23D context = 366D
        # Critic: 10 drones x (2 pos + 2 vel + 192 lidar) = 1960D
        # Padding: 10D
        # Total: 366 + 1960 + 10 = 2336D
        self.num_lidar_sectors = 64
        self.lidar_dim = self.num_lidar_sectors * 3  # 192D
        self.actor_dim = 366
        self.critic_dim = 1960
    ...
    def _observe(self, agent):
        ...
        lidar_64 = self._ray_cast_v20(idx) # 192D
        ...
        # 2. LiDAR (192D) - [64 sectors x 3 values from ray_cast_v20]
        ...
        obs_366 = np.concatenate([obs_self, lidar_64/8.0, np.concatenate(neighbor_slots), obs_context]).astype(np.float32)
        
        # 5. Global State (1960D)
        # 10 drones x 196D (Pos(2), Vel(2), LiDAR(192)) = 1960D
        per_drone_global = 2 + 2 + self.lidar_dim  # 196
        ...
        return np.concatenate([obs_366, global_state, padding_10])

    def _ray_cast_v20(self, idx):
        """64 sectors x 3 values (min_dist, dx, dy) = 192D.
        
        v20 upgrade: 4x angular resolution over v14.
        Each sector covers 5.625° (vs 22.5° in v14).
        This eliminates the spatial aliasing that caused
        blind-spot collisions in narrow geometric corridors.
        """
        num_sectors = self.num_lidar_sectors  # 64
        ...
        final = np.zeros(self.lidar_dim, dtype=np.float32)  # 192D
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            drone_clearance = 1.2 if spawn_mode == "clustered" else 1.65
            ...
            if gen_result is None: continue
    ...
    def _generate_obstacles(self, density, drone_clearance):
        ...
        for _ in range(MAX_OBSTACLE_ATTEMPTS):
            ...
            if is_rect:
                ...
                if np.linalg.norm(np.array([cx, cy]) - self.goal) <= r_eff + 2.0:
                    continue
                conflict = False
                ...
                if np.linalg.norm(np.array([cx, cy]) - self.goal) <= r + 2.0:
                    continue
    ...
    def _is_map_solvable(self, occupied, raster_res=0.1):
        gx = min(int(self.goal[0]/raster_res), occupied.shape[0]-1)
        gy = min(int(self.goal[1]/raster_res), occupied.shape[1]-1)
        ...
        for pos in self.positions:
            px = min(int(pos[0]/raster_res), occupied.shape[0]-1)
            py = min(int(pos[1]/raster_res), occupied.shape[1]-1)
