...
# ======================================================
#  PHASE B10: TOPOLOGICAL PATH GUIDANCE WITH WALL-GLIDE
#  Replaces Euclidean observations/rewards with Dijkstra
#  topological equivalents, and integrates an Adaptive
#  Wall-Glide vector field for stagnant drones (>40 steps).
# ======================================================
class SwarmLidarEnv_StepB10(ParallelEnv):
    metadata = {'render_modes': ['human'], "name": "swarm_lidar_stepB10_v0"}
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    if 0 <= nx < grid_size and 0 <= ny < grid_size:
                        if grid[nx, ny]:
                            # 8-way diagonal weights (0.2 cardinal, 0.28 diagonal)
    ...
    def get_shortest_path_direction(self, pos):
        ...
        # Fallback direction is raw straight-line goal vector
    ...
    def _observe(self, agent):
        ...
        # [v13 CTDE Drop-in Replacement] 
        # Replace Euclidean distance & Euclidean direction with 
        # Topological Shortest-path counterparts (maintains 650-dim weights compatibility)
        ...
        # ======================================================
        #  PHASE B10 UPGRADE: ADAPTIVE ONBOARD WALL-GLIDE BLENDING
        # ======================================================
        if self.steps_stagnant[agent] > 40:
            ...
            # Clockwise and Counter-Clockwise perpendicular wall-tangents
            t_cw = np.array([-np.sin(angle), np.cos(angle)], dtype=np.float32)
            t_ccw = np.array([np.sin(angle), -np.cos(angle)], dtype=np.float32)
            
            # Choose the tangent vector that aligns best with the Dijkstra direction
    ...
    def step(self, actions):
        ...
        for agent, action in actions.items():
            ...
            current_max = np.maximum(self.max_velocity * np.exp(-0.15 * neighbors_count), 1.10) 
        ...
        for agent in self.agents:
            ...
            # [v13 CTDE Reward] Goal Seeking Reward based on Topological Shortest Path Progress
            ...
            # Directional alignment bonus: reward moving along shortest-path direction
            ...
            # Check if blocked by a drone in front
            ...
            if path_dist < self.best_dist_to_goal[agent] - 0.1 and not is_stagnant:
                ...
                if is_blocked_by_drone:
                    ...
                    if blk <= 25:
                        self.steps_stagnant[agent] = 0
                        if speed < 0.2:
                            rewards[agent] += 0.5  # patience reward
                    else:
            ...
            # Front Clarity Reward + Near-Miss LiDAR Penalty
        ...
        if self.steps >= self.max_steps:
            for agent in self.agents:
                ...
                rewards[agent] -= 200.0  # Timeout terminal penalty
