...
from swarm_env_step_B5_v13_clean import SwarmLidarEnv_v13_Clean
from multiprocessing import Process, Pipe

# ======================================================
#  PHASE B5 v13-CLEAN: NOISELESS IDENTITY TRAINING (146D)
#  Architecture: Local 146D (Fixed Slots) | Global 400D
# ======================================================

class MAPPO_Extractor_v13(nn.Module):
    ...
    def forward(self, features): 
        return self.policy_net(features[:, :146]), self.value_net(features[:, 146:])
    def forward_actor(self, features): return self.policy_net(features[:, :146])
    def forward_critic(self, features): return self.value_net(features[:, 146:])

class MAPPO_Policy_v13(ActorCriticPolicy):
    def __init__(self, observation_space, action_space, lr_schedule, *args, **kwargs):
        super().__init__(observation_space, action_space, lr_schedule, *args, **kwargs)
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = MAPPO_Extractor_v13(self.features_dim, self.net_arch, self.activation_fn)

def worker(remote, parent_remote, density):
    parent_remote.close()
    env = SwarmLidarEnv_v13_Clean(target_density=density)
    n_drones = 10
    ghost_obs = {}
    total_obs_dim = 146 + 400
    try:
        while True:
...
class MultiProcessEnv_v13(VecEnv):
...
def run_v13_clean_training():
    print(f"🥇 LAUNCHING PHASE B5 v13-CLEAN: NOISELESS BASELINE (146D)")
    num_cpu = 10
    base_env = MultiProcessEnv_v13(n_workers=num_cpu, density=0.30)
    ...
    model = PPO(MAPPO_Policy_v13, env, verbose=1, 
                learning_rate=3e-4, 
                n_steps=2048, 
                batch_size=256, 
                n_epochs=10, 
                ...
                tensorboard_log="./logs/v13_Clean/")

    os.makedirs("./models/v13_Clean/", exist_ok=True)
    checkpoint = CheckpointCallback(save_freq=500_000, save_path='./models/v13_Clean/', name_prefix='v13_clean_cert')
    
    print("\n[V13-Clean] Starting 10,000,000 steps scratch training...")
    model.learn(total_timesteps=10_000_000, callback=checkpoint)
    
    model.save("./models/v13_Clean_Final_10M")
    env.save("./models/v13_Clean_Normalize.pkl")
    env.close()
    print(f"\n✅ Phase B5 v13-Clean Training Complete. Absolute Baseline Certified.")

if __name__ == "__main__":
    run_v13_clean_training()
