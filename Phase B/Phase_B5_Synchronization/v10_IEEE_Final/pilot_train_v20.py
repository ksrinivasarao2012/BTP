...
class CurriculumCallback_v20(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # Accelerated Curriculum for Pilot
        if ts < 30_000:
            prog = ts / 30_000.0
            density = 0.10 + (0.18 - 0.10) * prog
        elif ts < 60_000:
            prog = (ts - 30_000) / 30_000.0
            density = 0.18 + (0.26 - 0.18) * prog
        elif ts < 120_000:
            prog = (ts - 60_000) / 60_000.0
...
def run_v20_sensing_ablation():
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=10000,
        save_path='./models/v20_pilot_sensing/',
        name_prefix='v20_pilot'
    )
    ...
    model.learn(
        ...
        callback=[checkpoint_callback, log_std_cb, curriculum_cb, nan_check_cb, save_vec_cb, ent_schedule_cb, metrics_cb],
        progress_bar=False,
    ...
    env.close()
    model.save("v20_pilot_sensing_final")
    print("Pilot Training Complete and Saved: v20_pilot_sensing_final.zip")
