...
# This script supports BOTH 650D (v10-v14) and 752D (v17)
# Actor input dims:  v10-v14 => 130D | v17 => 222D
# Critic input dims: v10-v14 => 520D | v17 => 530D
# The obs dim is auto-detected from the loaded model.
# ======================================================

# ======================================================
#  IEEE-Grade K-Fold Evaluation Suite — v17 Master (752D)
# ======================================================
...
EVAL_MAX_STEPS = 800   # Max episode duration
...
MAPPO_Policy_v17     = make_policy_class(pi_in=222, vf_in=530)  # 752D total
...
def run_episode_batch_worker(args):
    ...
    if obs_dim == 752:
        ...
        env = SwarmLidarEnv_v19_GeometricDensity(render_mode=None, target_density=target_density)
        ...
        raise ValueError(f"[WORKER ERROR] Unknown obs dim {obs_dim}. Expected 650 or 752.")
    ...
    for ep in range(num_episodes):
        ...
        throughput = (ep_successes / step_count) * 100 if step_count > 0 else 0.0
...
def save_evaluation_plots(df, fold_df, timestamp, results_dir):
    try:
        ...
        ax.set_title(
            'v17 Master (752D Hybrid VFH) — Decentralized Swarm Performance\n'
        ...
        plt.tight_layout()
        bar_path = os.path.join(results_dir, f"v19_geometric_evaluation_chart_{timestamp}.png")
        ...
        ax2.set_title(
            ...
            'v17 Master (752D Hybrid VFH) | Phase B Benchmark',
        ...
        plt.tight_layout()
        box_path = os.path.join(results_dir, f"v19_geometric_boxplot_{timestamp}.png")
        plt.savefig(box_path, bbox_inches='tight')
        plt.close()
        print(f"[OK] Box plot saved: {box_path}", flush=True)

        # ---- FIGURE 3: Throughput Bar Plot ----
        t_means   = [r_df["Throughput"].mean(), c_df["Throughput"].mean()]
        t_ci      = [ci_halfwidth(r_df["Throughput"]), ci_halfwidth(c_df["Throughput"])]

        fig3, ax3 = plt.subplots(figsize=(6, 5), dpi=300)
        t_rects = ax3.bar([0, 1], t_means, width=0.35, yerr=t_ci,
                        color=['#008080', '#FF6F61'],
                        edgecolor='none', alpha=0.9, capsize=6,
                        error_kw=dict(ecolor='#333333', lw=1.5, capthick=1.5))
        for rect in t_rects:
            ...
            ax3.annotate(f'{h:.2f}', xy=(rect.get_x() + rect.get_width() / 2, h),
                        xytext=(0, 4), textcoords="offset points",
                        ha='center', va='bottom', fontsize=9.5, fontweight='bold')
        ax3.set_ylabel('Normalized Flow (Successes / 100 Steps)', fontsize=12, fontweight='bold')
        ax3.set_title('v17 Master (752D) | Throughput Comparison', fontsize=11, fontweight='bold', pad=16)
        ax3.set_xticks([0, 1])
        ax3.set_xticklabels(['Random Spawn', 'Cluster Spawn'], fontsize=11, fontweight='bold')
        ax3.grid(axis='y', linestyle='--', alpha=0.4)
        ax3.spines['top'].set_visible(False)
        ax3.spines['right'].set_visible(False)
        plt.tight_layout()
        t_path = os.path.join(results_dir, f"v19_geometric_throughput_{timestamp}.png")
        plt.savefig(t_path, bbox_inches='tight')
        plt.close(fig3)
        print(f"[OK] Throughput chart saved: {t_path}", flush=True)
    ...
    try:
        ...
        for csv_path in csv_files:
            try:
                ...
                stuck_agents = agent_max_steps[agent_max_steps >= (EVAL_MAX_STEPS - 1)]
...
def run_k_fold_master(model_path, cores=10, total_episodes=200, num_folds=10,
    ...
    results_dir = os.path.join("results", "v17")
    ...
    print("=" * 80, flush=True)
    print("  IEEE-GRADE K-FOLD VALIDATION SUITE | v17 Master (752D Hybrid VFH)", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"  Sensing Radii     : Environment defaults (100m unlimited for v17)", flush=True)
    ...
    for fold_idx in range(num_folds):
        ...
        for mode in ["random", "clustered"]:
            ...
            throughput     = (fold_stats["success"] / fold_drones) * (100.0 / avg_steps) if avg_steps > 0 else 0.0

            print(f"  Time: {elapsed:.1f}s | Avg Steps/Ep: {avg_steps:.1f} | Throughput: {throughput:.2f} (Successes / 100 Steps)", flush=True)
    ...
    print("\n" + "=" * 85, flush=True)
    print(f"  PHASE B MASTER (v17 752D VFH): K-FOLD FINAL SUMMARY", flush=True)
    ...
    for metric in ["Success_Rate", "Collision_Rate", "Timeout_Rate", "Throughput"]:
...
if __name__ == "__main__":
    # Default is the final v17 zip file
    default_model = r"d:\Swarm\BTP\models\v19_GeometricDensity_Final.zip"
    ...
    if not os.path.exists(m_path):
        ...
        print("Usage: python evaluate_v19_geometric.py <model_path> [cores] [eps_per_fold] [num_folds] [density]")
        sys.exit(1)
