...
# This script supports BOTH 650D (v10-v14) and 732D (v16)
# Actor input dims:  v10-v14 => 130D | v16 => 202D
# Critic input dims: v10-v14 => 520D | v16 => 530D
# The obs dim is auto-detected from the loaded model.
# ======================================================

# ======================================================
#  IEEE-Grade K-Fold Evaluation Suite — v16 Master (732D)
#
#  KEY DESIGN DECISIONS (fixes vs. previous k_fold scripts):
#  1. spawn_mode is ENFORCED (was silently ignored in v16 env)
...
# (12m LiDAR range), not controlled via set_curriculum. The v16 env defaults
...
#  v16     (732D): Actor=202D, Critic=530D
# ======================================================
...
MAPPO_Policy_v16     = make_policy_class(pi_in=202, vf_in=530)  # 732D total
...
def run_episode_batch_worker(args):
    ...
    if obs_dim == 732:
        ...
        env = SwarmLidarEnv_v16_Final(render_mode=None, target_density=target_density)
    ...
    for ep in range(num_episodes):
        ...
        # [FIX 1] Pass spawn_mode — now correctly read by the patched v16 env
...
def save_evaluation_plots(df, fold_df, timestamp, results_dir):
    try:
        ...
        ax.set_title(
            'v16 Master (732D) — Decentralized Swarm Navigation Performance\n'
        ...
        plt.tight_layout()
        bar_path = os.path.join(results_dir, f"v16_master_evaluation_chart_{timestamp}.png")
        ...
        ax2.set_title(
            ...
            'v16 Master (732D) | Phase B Benchmark',
        ...
        plt.tight_layout()
        box_path = os.path.join(results_dir, f"v16_master_boxplot_{timestamp}.png")
...
def run_k_fold_master(model_path, cores=10, total_episodes=200, num_folds=10,
    ...
    results_dir = os.path.join("results", "v16")
    ...
    print("=" * 80, flush=True)
    print("  IEEE-GRADE K-FOLD VALIDATION SUITE | v16 Master (732D)", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"  Sensing Radii     : Environment defaults (100m unlimited for v16; 12m LiDAR for v10-v14)", flush=True)
    ...
    # --------------------------------------------------
    print("\n" + "=" * 85, flush=True)
    print(f"  PHASE B MASTER (v16 732D): K-FOLD FINAL SUMMARY", flush=True)
...
if __name__ == "__main__":
    # Default: v16 Recovered Final model, 10 cores, 200 eps/fold, K=10, density=0.35
    default_model = "../../models/v16_Master_Recovered_Final.zip"
    ...
    if not os.path.isabs(m_path):
        ...
        workspace_model = r"d:\Swarm\BTP\models\v16_Master_Recovered_Final.zip"
    ...
    if not os.path.exists(m_path):
        ...
        print("Usage: python evaluate_v16_IEEE_Final.py <model_path> [cores] [eps_per_fold] [num_folds] [density]")
        sys.exit(1)
