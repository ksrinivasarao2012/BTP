...
# 3. Match v14 scaling perfectly to allow Warm Starting
code = code.replace("s_pos / R_SENSOR_NORM", "s_pos / 20.0")
code = code.replace("c_pos / R_COMM_NORM", "c_pos / 20.0")
code = code.replace("s_vel / V_MAX_NORM", "s_vel / 4.0")
code = code.replace("c_vel / V_MAX_NORM", "c_vel / 4.0")
code = code.replace("p_disc = np.linalg.norm(s_pos - c_pos) / R_SENSOR_NORM", "p_disc = np.linalg.norm(s_pos - c_pos) / 20.0")
code = code.replace("v_disc = np.linalg.norm(s_vel - c_vel) / (2*V_MAX_NORM)", "v_disc = np.linalg.norm(s_vel - c_vel) / 4.0")
...
print("v16 generated with EXACT v14 parity for warm starting.")
