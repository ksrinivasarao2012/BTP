...
def run_episode_batch_worker(args):
    ...
    from train_step_B5_sync_v11 import MAPPO_Policy_v11
    
    model = PPO.load(model_path, custom_objects={"policy_class": MAPPO_Policy_v11}, device="cpu")
    ...
    if obs_dim == 650:
        ...
        env = SwarmLidarEnv_StepB7(render_mode=None, target_density=target_density)
    else:
        raise ValueError(f"Unsupported model observation dimension: {obs_dim}. B7 evaluation requires a 650-dim model (v11).")
...
def save_evaluation_plot(df, timestamp, results_dir):
    try:
        ...
        ax.set_title('Decentralized Swarm Navigation Performance: Phase B7 Master (v11)', 
        ...
        png_filename = f"master_B7_evaluation_comparison_{timestamp}.png"
...
def run_k_fold_master(model_path, cores=10, total_episodes=200, num_folds=10):
    ...
    log_filename = f"master_B7_evaluation_log_{timestamp}.txt"
    ...
    csv_filename = f"master_B7_evaluation_results_{timestamp}.csv"
    ...
    print("\n" + "="*85, flush=True)
    print(f"           PHASE B7 MASTER: K-FOLD COMPARISON (K={num_folds}, {total_episodes} Eps/Fold)", flush=True)
    print("="*85, flush=True)
...
if __name__ == "__main__":
    default_model = "../../models/apex_ultra_patience_v11_final.zip"
