...
# ============================================================
#  V18 MASTER — FINE-TUNING FOR HIGH DENSITY (0.35)
#  Loads v17 checkpoint and specializes policy.
# ============================================================
...
BATCH_SIZE        = 128
N_EPOCHS          = 10
INITIAL_ENTROPY   = 0.01  # Lower entropy to encourage exploitation
FINAL_ENTROPY     = 0.001
CLIP_RANGE        = 0.10
LOG_STD_MIN       = -3.0
LOG_STD_MAX       = 0.0
...
CHECKPOINT_DIR    = r"d:\Swarm\BTP\models\v18_checkpoints"
FINAL_MODEL_PATH  = r"d:\Swarm\BTP\models\v18_Master_Finetuned.zip"
NORM_SAVE_PATH    = r"d:\Swarm\BTP\models\v18_Master_Normalize.pkl"

LOAD_MODEL_PATH   = r"d:\Swarm\BTP\models\v17_Master_Final.zip"
LOAD_NORM_PATH    = r"d:\Swarm\BTP\models\v17_Master_Normalize.pkl"
...
class CurriculumCallback_v18(BaseCallback):
    def __init__(self, verbose=0):
        super().__init__(verbose)
    ...
    def _on_step(self) -> bool:
        ...
        r_sensor = 100.0  # Unlimited sensing
        
        # Hard lock density for fine-tuning
        density = 0.35
        
        # Hard lock communication radius for benchmark specialization
        r_comm = 10.0
        ...
        if (abs(r_comm - self._last_r_comm) > 0.1 or abs(density - self._last_density) > 0.005):
            ...
            if self.verbose > 0:
                print(f"  [Fine-Tuning] Density LOCKED = {density:.2f} | R_comm LOCKED = {r_comm:.1f}", flush=True)
        return True
...
class MultiProcessEnv_v18(VecEnv):
...
def run_v18_finetuning():
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    os.makedirs(os.path.dirname(FINAL_MODEL_PATH), exist_ok=True)

    print("======================================================================")
    print("  V18 FINE-TUNING — SPECIALIZING FOR HIGH DENSITY (0.35)")
    print("======================================================================")
    ...
    base_env = MultiProcessEnv_v18(n_workers=NUM_WORKERS)
    ...
    if os.path.exists(LOAD_NORM_PATH):
        ...
        raise FileNotFoundError(f"VecNormalize file missing: {LOAD_NORM_PATH}. Cannot fine-tune correctly without it.")
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        name_prefix="v18_master_ckpt"
    )
    log_std_cb   = LogStdClampCallback()
    curriculum_cb = CurriculumCallback_v18()
    ...
    print(f"\n[DONE] Saving final fine-tuned model to: {FINAL_MODEL_PATH}", flush=True)
