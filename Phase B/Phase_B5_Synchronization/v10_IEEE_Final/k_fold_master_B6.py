...
def run_episode_batch_worker(args):
    ...
    from train_step_B5_sync_v10 import MAPPO_Policy_v10_Pro
    
    model = PPO.load(model_path, custom_objects={"policy_class": MAPPO_Policy_v10_Pro}, device="cpu")
    ...
    if obs_dim == 650:
        from swarm_env_step_B6 import SwarmLidarEnv_StepB6
        env = SwarmLidarEnv_StepB6(render_mode=None, target_density=target_density)
    else:
        raise ValueError(f"Unsupported model observation dimension: {obs_dim}. B6 evaluation requires a 650-dim model (v10).")

    # Set up fold directory structures (clustered spawns are saved in 'dense' folder to match standards)
...
def save_evaluation_plot(df, timestamp, results_dir):
    try:
        ...
        ax.set_title('Decentralized Swarm Navigation Performance: Phase B6 Master (v10)', 
        ...
        png_filename = f"master_B6_evaluation_comparison_{timestamp}.png"
...
def run_k_fold_master(model_path, cores=10, total_episodes=200, num_folds=10):
    ...
    log_filename = f"master_B6_evaluation_log_{timestamp}.txt"
    ...
    # Save results to CSV directly in results/v10/
    df = pd.DataFrame(results_data)
    csv_filename = f"master_B6_evaluation_results_{timestamp}.csv"
    csv_path = os.path.join(main_results_dir, csv_filename)
    df.to_csv(csv_path, index=False)
    print(f"[OK] Results successfully saved to: {csv_path}", flush=True)
    
    # Generate and save comparison plot inside results/v10/model_base/
    ...
    print("\n" + "="*85, flush=True)
    print(f"           PHASE B6 MASTER: K-FOLD COMPARISON (K={num_folds}, {total_episodes} Eps/Fold)", flush=True)
    print("="*85, flush=True)
...
if __name__ == "__main__":
    default_model = "../../models/apex_ultra_queue_v10_final.zip"
