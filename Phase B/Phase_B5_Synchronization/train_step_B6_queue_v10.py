...
# ======================================================
#  PHASE B6 v10: Smart Queuing & Directional Yielding
# ======================================================
...
def worker(remote, parent_remote, density, drone_radius, safety_radius):
    ...
    os.environ["OMP_NUM_THREADS"] = "1"
    parent_remote.close()
    env = SwarmLidarEnv_StepB6(render_mode=None, target_density=density)
    n_drones = 10
    ghost_obs = {}
    try:
        while True:
...
def run_v10_training():
    num_cpu = 10 
    print(f"PHASE B6 v10: Smart Queuing Training from Scratch")
    ...
    # Train from scratch (no weights loaded) so network can adapt to 12m LiDAR natively
    print(f"Initializing fresh weights for v10...")
    model = PPO(MAPPO_Policy_B5, env, ent_coef=0.03, learning_rate=3e-4, n_steps=2048, batch_size=256, verbose=1)
    
    # 15M Step Hardening Curriculum
    curriculum = [
        (5_000_000, 0.25),   # Phase 1: Base Navigation & 12m LiDAR awareness
        (10_000_000, 0.35),  # Phase 2: Smart Queuing in Choke Points
    ]
    total_steps = 0
    os.makedirs("./models/checkpoints_b6v10/", exist_ok=True)
    checkpoint_callback = CheckpointCallback(save_freq=500_000, save_path='./models/checkpoints_b6v10/', name_prefix='b6v10')
    ...
    for steps, density in curriculum:
        ...
        total_steps += steps
        model.save(f"./models/apex_ultra_queue_v10_mid_{total_steps//1000}k")
    
    model.save("./models/apex_ultra_queue_v10_final")
    env.save("./models/vecnormalize_queue_v10_final.pkl")
    env.close()
    print(f"\nPhase B6 v10 Training Complete ({total_steps/1e6:.1f}M steps). Model saved.")
