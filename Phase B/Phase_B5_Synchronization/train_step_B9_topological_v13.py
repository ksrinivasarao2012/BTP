...
from swarm_env_step_B9 import SwarmLidarEnv_StepB9
from multiprocessing import Process, Pipe

# ======================================================
#  PHASE B9 v13: Topological Path Guidance Fine-Tuning
#  Fine-tunes from B8 v12 weights in the new B9 environment.
# ======================================================
...
# -----------------------------------------------------------
# Subprocess Worker – uses SwarmLidarEnv_StepB9
# -----------------------------------------------------------
def worker(remote, parent_remote, density, drone_radius, safety_radius):
    ...
    os.environ["OMP_NUM_THREADS"] = "1"
    parent_remote.close()
    env = SwarmLidarEnv_StepB9(render_mode=None, target_density=density)
    n_drones = 10
    ghost_obs = {}
    try:
        while True:
...
class MultiProcessPZEnv_B9(VecEnv):
...
def run_v13_training():
    num_cpu = 10
    v12_model_path = "models/apex_ultra_positional_v12_final.zip"
    
    # Robust path resolution
    possible_paths = [
        v12_model_path,
        "../../models/apex_ultra_positional_v12_final.zip",
        "../../../models/apex_ultra_positional_v12_final.zip",
        os.path.join(os.path.dirname(__file__), "../../models/apex_ultra_positional_v12_final.zip"),
        os.path.join(os.path.dirname(__file__), "../../../models/apex_ultra_positional_v12_final.zip")
    ]
    ...
    print(f"PHASE B9 v13: Topological Path Guidance Fine-Tuning")
    print(f"  -> Loading pre-trained v12 weights from: {v12_model_path}")

    # Create the new B9 environment
    base_env = MultiProcessPZEnv_B9(n_workers=num_cpu, density=0.35)
    env = VecNormalize(base_env, norm_obs=False, norm_reward=True, clip_reward=10.0)

    # Load pre-trained v12 model and attach to the new environment
    model = PPO.load(
        v12_model_path,
        env=env,
    ...
    print(f"  -> v12 weights loaded successfully. Fine-tuning in B9 environment...")

    # Curriculum:
    # Phase 1: 1M @ 0.30 density
    # Phase 2: 2M @ 0.35 density
    curriculum = [
        (1_000_000, 0.30),   # Phase 1: Adaptation
        (2_000_000, 0.35),   # Phase 2: Full-density detour mastery
    ]

    os.makedirs("./models/checkpoints_b9v13/", exist_ok=True)
    checkpoint_callback = CheckpointCallback(
        save_freq=500_000,
        save_path='./models/checkpoints_b9v13/',
        name_prefix='b9v13'
    )
    ...
    for steps, density in curriculum:
        ...
        model.save(f"./models/apex_ultra_topological_v13_mid_{steps//1_000_000}M")

    model.save("./models/apex_ultra_topological_v13_final")
    env.save("./models/vecnormalize_topological_v13_final.pkl")
    env.close()
    print(f"\nPhase B9 v13 Training Complete (3M fine-tune steps). Model saved.")
