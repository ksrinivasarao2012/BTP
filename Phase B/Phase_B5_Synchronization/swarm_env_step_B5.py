...
# ======================================================
#  PHASE B5 v2: SWARM SYNCHRONIZATION + RESTORED REWARDS
#  Fixed: Map Solvability, Yielding, Near-Miss, Timeout Tag
# ======================================================
class SwarmLidarEnv_StepB5(ParallelEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            speed = np.linalg.norm(self.velocities[idx])
            alignment = np.dot(self.velocities[idx] / (speed + 1e-6), (self.goal - pos) / (dist_goal + 1e-6))
            ...
            for j in range(self.n_drones):
                ...
                # [v3 FIX] Urgent Proximity Repulsion (dominates goal-seeking at close range)
                if dist < 0.5:
                ...
                if dist < 1.5:
                    v_self_norm = np.linalg.norm(self.velocities[idx])
                    v_neigh_norm = np.linalg.norm(self.velocities[j])
                    if v_self_norm > 0.1 and v_neigh_norm > 0.1:
                        cos_sim = np.dot(self.velocities[idx], self.velocities[j]) / (v_self_norm * v_neigh_norm)
                        goal_progress = max(0.0, np.dot(self.velocities[idx] / v_self_norm, unit_to_goal))
