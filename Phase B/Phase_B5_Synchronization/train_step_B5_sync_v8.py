    ...
    # ======================================================
    #  PHASE B5 v8: Surgical Reward Masking Fine-Tuning
    # ======================================================
    ...
    def run_sync_training():
        num_cpu = 10 
        print(f"PHASE B5 v8: Surgical Reward Masking (Fine-tuning from v6)...")
        ...
        # Register custom policy to PPO load doesn't fail
        custom_objects = {"policy_class": MAPPO_Policy_B5}
        model = PPO.load(model_path, env=env, ent_coef=0.01, learning_rate=2e-5, custom_objects=custom_objects)
        
        # 2M Step Surgical Fine-Tuning
        curriculum = [
            (1_000_000, 0.30),   # Phase 1: Re-learn retreat behavior under free-masking
            (1_000_000, 0.35),   # Phase 2: Final stress hardening
        ]
        total_steps = 0
        os.makedirs("./models/checkpoints_b5v8/", exist_ok=True)
        checkpoint_callback = CheckpointCallback(save_freq=500_000, save_path='./models/checkpoints_b5v8/', name_prefix='b5v8_mask')
        for steps, density in curriculum:
            ...
            total_steps += steps
            model.save(f"./models/apex_ultra_sync_v8_mid_{total_steps//1000}k")
        model.save("./models/apex_ultra_sync_v8_final")
        env.save("./models/vecnormalize_sync_v8_final.pkl")
        env.close()
        print(f"\nPhase B5 v8 Training Complete ({total_steps/1e6:.1f}M steps). Model saved.")
