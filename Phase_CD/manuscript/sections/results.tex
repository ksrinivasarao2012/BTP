...
curve of $0.99$ under oracle association and $0.85$--$0.90$ under realistic
nearest-obstacle association.
...
($0.66$--$0.98$). The remaining three knobs are likewise self-defeating.
Per-frame jitter is zero-mean and so does not shift the temporal mean; it
only raises variance, and in fact reduces the attacker's own harm faster
than it dents recovery. Intermittent broadcasting (duty) dilutes the
attacker's evidence but equally dilutes its harm: at the lowest duty the
harm falls to $+5.0$ pp while recall falls to $0.41$. A surface gap large
