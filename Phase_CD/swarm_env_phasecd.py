...
class SwarmLidarEnv_StepB10_8_0m(ParallelEnv):
    ...
    def __init__(self, render_mode=None, target_density=0.20, drone_radius=0.15, safety_radius=0.19, communication_range=8.0, use_congestion=True, congestion_mode="env", lidar_range=12.0):
        ...
        # collab_nearest_only: each comm-neighbor shares ONLY its single nearest obstacle (Design A's
        # 1-per-neighbor channel) instead of all it senses -> the restricted oracle for Check 0.
        self.collab_nearest_only = False
    ...
    def _ray_cast(self, agent_idx):
        ...
        if self.obstacles:
            ...
            if collab:
                # collaborative perception: ego senses all obstacles within its own lidar_range; each
                # comm-neighbor contributes the obstacles IT senses. collab_nearest_only=True -> each
                # neighbor shares ONLY its single nearest obstacle (= Design A's per-neighbor channel
                # content), so this restricted oracle upper-bounds Design A WITHOUT training.
                centers = obs_array[:, :2]
                keep = np.linalg.norm(centers - pos, axis=1) <= self.lidar_range
                for j in range(self.n_drones):
                    if j != agent_idx and self.possible_agents[j] in self.agents \
                        ...
                        if self.collab_nearest_only:
                            k = int(np.argmin(dj))
                            if dj[k] <= self.lidar_range:
                                keep[k] = True
                        else:
