"""
STAGE 3 — Train a SHORT-LiDAR policy that is forced to use LiDAR + goal + COMMUNICATION.

Clone of train_comm.py, with the LiDAR sensing range now a CLI argument. Transfers from M0
(apex_ultra_glide_v14_comm8_lidar_final), congestion_mode="lidar" (CTDE-clean, matches M0),
same curriculum as the comm sweep ((2M,0.30) -> (3M,0.35)).

This is THE GATE: train the ablation PAIR at one lidar_range and compare no-adversary success:
    comm-ON  :  python train_shortlidar.py 5 10      (lidar=5 m, comm=10 m)
    comm-OFF :  python train_shortlidar.py 5 0       (lidar=5 m, comm=0  -> neighbor block = zeros)
If comm-ON beats comm-OFF by >= ~5-10 pp, communication carries non-redundant value -> poisonable
-> proceed to the trust defense. If ~0, comm is redundant even under partial observability -> stop.

Usage:
    python train_shortlidar.py <lidar_range> <comm_range>
Produces: models/short_lidar{L}_comm{C}_final.zip  (+ vecnormalize pkl)
"""
import os
...
# --- run from anywhere: this folder + repo root on path (also for spawned workers); paths -> root ---
...
CONGESTION_MODE = "lidar"   # CTDE-clean, matches the M0 transfer base
OBS_DIM = 130 + 520
...
def worker(remote, parent_remote, density, comm_range, lidar_range, congestion_mode):
    import os
    os.environ["KMP_DUPLICATE_LIB_OK"] = "True"
    os.environ["OMP_NUM_THREADS"] = "1"
    parent_remote.close()
...
class MultiProcessPZEnv(VecEnv):
    def __init__(self, n_workers, density, comm_range, lidar_range, congestion_mode):
        self.closed = False
        self.n_workers = n_workers
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(n_workers)])
        self.ps = [Process(target=worker, args=(wr, r, density, comm_range, lidar_range, congestion_mode), daemon=True)
...
def find_base():
    for p in ["models/apex_ultra_glide_v14_comm8_lidar_final.zip",
        ...
        if os.path.exists(p):
            return p
    raise FileNotFoundError("transfer base model not found.")


def main():
    if len(sys.argv) < 3:
        print("Usage: python train_shortlidar.py <lidar_range> <comm_range>")
        print("  comm-ON  : python train_shortlidar.py 5 10")
        print("  comm-OFF : python train_shortlidar.py 5 0")
        sys.exit(1)
    ...
    tag = "OFF" if comm_range == 0 else "ON"

    base = find_base()
    print(f"STAGE 3 — short-LiDAR train | lidar={lidar_range} m | comm={comm_range} m ({tag}) | congestion={CONGESTION_MODE}")
    ...
    curriculum = [(2_000_000, 0.30), (3_000_000, 0.35)]   # identical to the comm sweep
    ckpt_dir = f"./models/checkpoints_short_lidar{L}_comm{C}/"
    os.makedirs(ckpt_dir, exist_ok=True)
    cb = CheckpointCallback(save_freq=500_000, save_path=ckpt_dir, name_prefix=f"short_lidar{L}_comm{C}")

    for steps, density in curriculum:
        print(f"\nPHASE: density={density} | steps={steps/1e6:.1f}M | lidar={lidar_range}m | comm={comm_range}m")
    ...
    out = f"./models/short_lidar{L}_comm{C}_final"
    model.save(out)
    env.save(f"./models/vecnormalize_short_lidar{L}_comm{C}_final.pkl")
    env.close()
    print(f"\nDone. Model saved: {out}.zip")
    print(f"Next: evaluate no-adversary success and compare comm-ON vs comm-OFF (REVIEW CHECKPOINT 2).")
