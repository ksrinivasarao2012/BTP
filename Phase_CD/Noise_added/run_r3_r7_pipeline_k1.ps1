# ============================================================
#  R3 (comm-loss) + R7 (density) rebuttal experiments — f=1 traitor.
#  Same protocol as run_r3_r7_pipeline.ps1 (k=2 default) but for k=1.
#  Filenames carry a _k1 suffix so they do NOT collide with the k=2 logs.
#  Est. ~10-12 h (same scope as the k=2 run: comm-loss ~9-10h + density ~2.5h).
#  DO NOT run this at the same time as the k=2 or k=3 pipeline — all three
#  want the full 10 cores; run them one after another.
#  Run:  Phase_CD\Noise_added\run_r3_r7_pipeline_k1.bat
# ============================================================
...
Write-Host " R3+R7 REBUTTAL (k=1) | maps=$Maps | ${Workers}w | started $t0" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

Go "R3 comm-loss camouflage (k=1)" "$out\comm_loss_camouflage_${Maps}_k1.txt" `
   { & $py Phase_CD\Noise_added\eval_comm_loss.py $M2 $Maps $K $Workers camouflage }

Go "R7 density camouflage (k=1)" "$out\density_sweep_camouflage_${Maps}_k1.txt" `
   { & $py Phase_CD\Noise_added\eval_density_sweep.py $M2 $Maps $K $Workers camouflage }

$dt = (Get-Date) - $t0
Write-Host "`n============================================================" -ForegroundColor Green
Write-Host (" R3+R7 (k=1) COMPLETE in {0:hh\:mm\:ss}.  Logs: {1}\" -f $dt, $out) -ForegroundColor Green
