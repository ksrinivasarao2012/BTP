"""
B2 — Surgery for the raster architecture: expand M0's actor input 130 -> 178 (zero-init the 48 new
shared-map weights) so the expanded model == M0 when the shared block is zero. Only
mlp_extractor.policy_net.0.weight changes; everything else copied verbatim.

Usage: python surgical_expand_raster.py [m0_path] [out_path]
Defaults: models/apex_ultra_glide_v14_comm8_lidar_final.zip -> models/raster_expanded_M0.zip
"""
import os
...
LOCAL_OLD = 130
LOCAL_NEW = 178          # 130 + 48 shared-map
GLOBAL = 520
OBS_NEW = LOCAL_NEW + GLOBAL   # 698
M0_DEFAULT = "models/apex_ultra_glide_v14_comm8_lidar_final.zip"
OUT_DEFAULT = "models/raster_expanded_M0.zip"
EXPAND_KEY = "mlp_extractor.policy_net.0.weight"


class MAPPO_Extractor_B5(nn.Module):  # OLD (130)
...
class MAPPO_Extractor_Raster(nn.Module):  # NEW (178)
    def __init__(self, features_dim, net_arch, activation_fn):
        super().__init__()
...
class _Stub(gym.Env):
    ...
    def reset(self, *, seed=None, options=None): return np.zeros(OBS_NEW, np.float32), {}
    def step(self, action): return np.zeros(OBS_NEW, np.float32), 0.0, False, False, {}
...
def main():
    ...
    new = PPO(MAPPO_Policy_Raster, _Stub(),
              policy_kwargs={"net_arch": m0.policy.net_arch, "activation_fn": m0.policy.activation_fn}, device="cpu")

    m0_sd = m0.policy.state_dict(); new_sd = new.policy.state_dict()
    copied = expanded = 0; mismatched = []
    for k in list(new_sd.keys()):
        if k == EXPAND_KEY and k in m0_sd:
            w = torch.zeros_like(new_sd[k]); w[:, :LOCAL_OLD] = m0_sd[k]; new_sd[k] = w; expanded += 1
    ...
    print(f"[*] copied {copied} | expanded {expanded} ({EXPAND_KEY})  first-130==M0: {ok_copy} | new 48 cols zero: {ok_zero}")
    if mismatched:
        print(f"    [!] unexpected mismatched keys: {mismatched}")
    if not (ok_copy and ok_zero and expanded == 1 and not mismatched):
        print("[!] surgery sanity FAILED."); return
    new.save(out_path)
    print(f"\n[OK] saved: {out_path}")
