...
# [V15 Fixed] Two changes from v15 master:
#   1. _generate_obstacles: obstacle surface now stays >= MIN_WALL_GAP (0.20m) from field boundary
#   2. Density ceiling raised to 0.30 (validated by geometric feasibility sweep)
# Everything else identical to v15 master.

R_SENSOR_NORM = 8.0
R_COMM_NORM   = 10.0
V_MAX_NORM    = 2.0

MIN_WALL_GAP  = 0.20  # obstacle surface to field boundary clearance

class SigmoidLUT:
    def __init__(self, x_min=-10.0, x_max=10.0, resolution=2000):
        self.x_min = x_min; self.x_max = x_max; self.resolution = resolution
...
class SwarmEnvFixed(ParallelEnv):
    metadata = {'render_modes': ['human'], "name": "swarm_env_fixed"}

    def __init__(self, render_mode=None, target_density=0.15):
        super().__init__()
        ...
        self.fallback_spawn_count    = 0  # times a drone was placed without obstacle check
        self.close_spawn_count       = 0  # times a drone spawned within 7m of goal
        self.unreachable_spawn_count = 0  # times a drone spawned with no valid path to goal
    ...
    def _compute_shortest_path_distance_map(self):
        ...
        clearance_radius = 2 * self.drone_radius + 0.10  # 0.40m — consistent with solvability BFS
        for ox, oy, orad in self.obstacles:
            ...
            for gx in x_range:
                for gy in y_range:
                    cx = gx*grid_resolution + grid_resolution/2
                    cy = gy*grid_resolution + grid_resolution/2
                    if np.sqrt((cx-ox)**2+(cy-oy)**2) < (orad+clearance_radius):
                        grid[gx, gy] = False
    ...
    def get_shortest_path_distance(self, pos):
        ...
        return dist if dist < 999.0 else np.linalg.norm(self.goal-pos)
    ...
    def get_shortest_path_direction(self, pos):
        ...
        best_dir = best_dir / (np.linalg.norm(best_dir) + 1e-5)
        if min_dist < 999.0:
    ...
    def _prepare_broadcasts(self):
        self.broadcasts = {
            i: {'pos': self.positions[i].copy(), 'vel': self.velocities[i].copy(),
        ...
        for j in range(self.n_drones):
            if f"drone_{j}" in self.agents:
                ...
                self.global_state[j*52:(j+1)*52] = np.concatenate([
                    self.positions[j]/self.WIDTH, self.velocities[j]/V_MAX_NORM, g_lid
                ])
    ...
    def _ray_cast(self, idx):
        ...
        if len(self.obstacles) > 0:
            min_dists = np.minimum(min_dists, intersect_circles(
                self.obstacles_array[:,:2], self.obstacles_array[:,2]+self.drone_radius))

        others = [j for j in range(self.n_drones) if j!=idx and f"drone_{j}" in self.agents]
        if others:
            radii = self.others_radii.get(len(others), np.full(len(others), 2*self.drone_radius, dtype=np.float32))
    ...
    def _observe(self, agent):
        ...
        for j in range(self.n_drones):
            ...
            if f"drone_{j}" in self.agents:
                ...
                slot[0:2] = s_pos/R_SENSOR_NORM; slot[2:4] = s_vel/V_MAX_NORM
                slot[4]   = is_vis
                slot[5:7] = c_pos/R_COMM_NORM;   slot[7:9] = c_vel/V_MAX_NORM
        ...
        congestion = sum(1 for j in range(self.n_drones)
                         if j!=idx and f"drone_{j}" in self.agents and self.dist_matrix[idx,j]<1.0) / self.n_drones
    ...
    def _generate_obstacles(self, sc):
        ...
        obs = []
        raster_res = 0.05
        rw = int(self.WIDTH/raster_res); rh = int(self.HEIGHT/raster_res)
        ...
        for _ in range(2000):
            if cur >= target: break
            ch = np.random.random()
            if ch < 0.2:   r = np.random.uniform(1.5, 2.5)
            elif ch < 0.6: r = np.random.uniform(0.6, 1.4)
            else:          r = np.random.uniform(0.2, 0.5)

            # FIX: obstacle surface stays >= MIN_WALL_GAP from field boundary
            lo_x, hi_x = r + MIN_WALL_GAP, self.WIDTH  - r - MIN_WALL_GAP
            lo_y, hi_y = r + MIN_WALL_GAP, self.HEIGHT - r - MIN_WALL_GAP
            if lo_x >= hi_x or lo_y >= hi_y: continue
            cx = np.random.uniform(lo_x, hi_x)
            cy = np.random.uniform(lo_y, hi_y)

            if np.linalg.norm([cx,cy]-self.goal) <= r+2.0:  continue
            if np.linalg.norm([cx,cy]-sc)        <= r+2.0:  continue

            xmin = max(0, int((cx-r)/raster_res)); xmax = min(rw, int((cx+r)/raster_res)+1)
            ymin = max(0, int((cy-r)/raster_res)); ymax = min(rh, int((cy+r)/raster_res)+1)
    ...
    def _is_map_solvable(self, start_pos, grid_res=0.2):
        gs = int(np.ceil(self.WIDTH/grid_res)); grid = np.ones((gs,gs), dtype=bool)
        clr = 2 * self.drone_radius + 0.10  # matches density sweep BFS clearance (0.40m)
        for ox,oy,orad in self.obstacles:
            ...
            for gx in range(xm,xM):
                for gy in range(ym,yM):
                    if np.sqrt((gx*grid_res+grid_res/2-ox)**2+(gy*grid_res+grid_res/2-oy)**2)<(orad+clr):
                        grid[gx,gy]=False
    ...
    def reset(self, seed=None, options=None):
        ...
        self.goal = np.array(options["goal"]) if (options and "goal" in options) \
                    else np.array([np.random.uniform(2.0,18.0), np.random.uniform(2.0,18.0)])
        for _ in range(200):
        ...
        is_cl = (options.get("spawn_mode")=="clustered") if options and "spawn_mode" in options \
                else np.random.random() < 0.5
        self.current_spawn_mode = "clustered" if is_cl else "scattered"
        ...
        for i in range(self.n_drones):
            ...
            for search_radius in search_radii:
                for _ in range(150):
                    p = np.random.uniform(sc-search_radius, sc+search_radius, 2) if is_cl \
                        else np.random.uniform(1.0, 19.0, 2)
                    p = np.clip(p, 0.6, 19.4)
                    if (all(np.linalg.norm(p-self.positions[j])>=0.50 for j in range(i)) and
                        all(np.linalg.norm(p-np.array([ox,oy]))>=(0.45+orad) for ox,oy,orad in self.obstacles)):
                        self.positions[i], pl = p, True; break
                if pl: break
            if not pl:
                angle = i * (2.0 * np.pi / self.n_drones)
                for r_dist in [0.60, 1.20, 1.80, 2.50, 3.50]:
                    p = np.clip(sc + np.array([r_dist*np.cos(angle), r_dist*np.sin(angle)], dtype=np.float32), 0.6, 19.4)
                    if all(np.linalg.norm(p - np.array([ox,oy])) >= (0.45+orad) for ox,oy,orad in self.obstacles):
                        self.positions[i] = p; pl = True; break
                if not pl:  # absolute last resort — no obstacle check
                    self.positions[i] = np.clip(sc + np.array([0.60*np.cos(angle), 0.60*np.sin(angle)], dtype=np.float32), 0.6, 19.4)
                    self.fallback_spawn_count += 1

        for i in range(self.n_drones):
            if np.linalg.norm(self.positions[i] - self.goal) < 7.0:
                self.close_spawn_count += 1
    ...
    def step(self, actions):
        ...
        for agent, act in actions.items():
            ...
            if self.positions[idx][0] <= 0.0 or self.positions[idx][0] >= 20.0:
                self.velocities[idx][0] = 0.0
            if self.positions[idx][1] <= 0.0 or self.positions[idx][1] >= 20.0:
        ...
        for a in self.agents:
            ...
            for j in range(self.n_drones):
                ...
                d = self.dist_matrix[idx,j]; rp = self.positions[j]-pos; rv = self.velocities[idx]-self.velocities[j]
                ...
                if d<1.0 and self.steps_stagnant.get(f"drone_{j}",0)>30:
                    r -= 50.0*GLOBAL_SIGMOID_LUT(-10.0*(d-0.5))
                if d<1.5:
            ...
            act = actions.get(a, np.zeros(2))
            if a in self.last_actions: r -= 0.05*np.linalg.norm(act-self.last_actions[a])**2
            self.last_actions[a] = act.copy() if hasattr(act,'copy') else np.array(act)

            ld = self.lidar_cache[a]; fm = (ld[31]+ld[16]+ld[17])/3.0
            r += (fm/R_SENSOR_NORM)*0.2
            ml = np.min(ld[:16])
            if ml < 0.15: r -= ((0.15-ml)/0.15)**2

            hw = min(pos[0], 20-pos[0], pos[1], 20-pos[1]) <= 0.05
            ho = any(np.linalg.norm(pos-np.array([ox,oy])) < 0.15+orad for ox,oy,orad in self.obstacles)
            hd = any(self.dist_matrix[idx,j]<0.3 for j in range(self.n_drones) if j!=idx and f"drone_{j}" in self.agents)

            if hw or ho or hd:
                r, terms[a] = -150.0, True
                self.infos[a]["cause"] = "collision"
                self.infos[a]["spawn_mode"] = self.current_spawn_mode
            elif path_dist < 0.75:
                r += 500.0+(100.0/(1.0+sp)); terms[a]=True
                self.infos[a]["cause"] = "success"
                self.infos[a]["spawn_mode"] = self.current_spawn_mode
            else:
                terms[a] = False
