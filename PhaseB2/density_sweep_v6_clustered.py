...
486 parameter combinations × 5 densities × 100 maps = 243,000 total evaluations.
"""
...
CLUSTER_RADII = [1.5, 2.0, 2.5]
SPAWN_OBSTACLE_CLEARANCES = [0.25, 0.30]
SC_GOAL_MIN_DISTS = [6.0, 7.0, 8.0]
GOAL_SPAWN_CLEARANCES = [4.0, 5.0, 6.0]
INTER_DRONE_MINS = [0.20, 0.25, 0.30]
GOAL_EXCLUSION_RADII = [0.60, 0.65, 0.70]
...
def main():
    ...
    print(f"Expected runtime: 2-4 hours with {NUM_WORKERS} workers\n")

    # Output CSV setup
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = Path(__file__).parent / f"density_sweep_v6_results_{timestamp}.csv"
